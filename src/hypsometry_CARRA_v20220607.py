...
(ax3, ax1) = gs.subplots(sharex='col', sharey='row')
...
for jj in range(0,len(label)-1):  #len(label)#CARRA and ERA5
    ...
    if jj==2:  #MAR
        MAR_BS15=1 #choose with BS (1) or without (0)
        # ni=446; nj=240 #6km
        ...
        fn=raw_path+'MARv3.12.0.4-noBS-15km-yearly-ERA5-1998-2020.nc' #15km new version
        if MAR_BS15: fn=raw_path+'MARv3.12.0.4-BS-15km-yearly-ERA5-1998-2020.nc' #15km new version
        ds=xr.open_dataset(fn)
...
for jj in range(0,len(label)-1):  #len(label)#CARRA and ERA5
    ...
    idx1 = (np.abs(np.cumsum(tp_cum) - (np.cumsum(tp_cum)[-1]/2))).argmin() #find elev nearest to 50% mark
    idx2 = (np.abs(np.cumsum(areas)/1e6 - (np.cumsum(areas)[-1]/(2*1e6)))).argmin() #find elev nearest to 50% mark
...
mult=1.4
#text (a) and (b)
ax1.text(-1.13,1.13,'(a)',transform=ax1.transAxes, fontsize=fs*mult,
    verticalalignment='top',rotation=0,color='k', rotation_mode="anchor")
ax3.text(0.05,1.13,'(b)',transform=ax1.transAxes, fontsize=fs*mult,
    verticalalignment='top',rotation=0,color='k', rotation_mode="anchor")
mult=1
#text median
ax3.text(-.99,.6,'50th percentiles',transform=ax1.transAxes, fontsize=fs*mult,
    verticalalignment='top',rotation=0,color='k', rotation_mode="anchor")
